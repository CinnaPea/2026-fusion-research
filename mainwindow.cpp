...
void MainWindow::applyFiltersAndModes()
{
    ...
    if (viewMode == "sample") {
        int count = ui->spnSampleCount->value();
        qart::core::visualization::PreviewPairSelector selector;
        qart::core::visualization::PreviewSelectionRequest request(count, {});
        try {
...
void MainWindow::displayPair(const ValidationManifestRow &row)
{
    ...
    PairValidationStatus status = qart::core::domain::validation::pairValidationStatusFromString(row.status());
